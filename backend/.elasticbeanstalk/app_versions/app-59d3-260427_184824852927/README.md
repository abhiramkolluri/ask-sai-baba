...
AI-powered search and query system for Sai Baba discourses using Weaviate hybrid search and direct OpenAI integration.
...
- 💾 **Conversation Memory**: Stores chat history directly in Weaviate for fast retrieval
- 🔐 **Google OAuth 2.0 Integration**: Secure social authentication
...
- **Database**: Weaviate (hybrid search + user data/conversations)
- **AI/ML**: OpenAI (direct SDK for embeddings & chat)
- **Authentication**: Google OAuth 2.0
- **Infrastructure**: AWS CDK, API Gateway (OpenAPI 3.0 driven)
- **Deployment**: AWS Elastic Beanstalk, Docker, Railway, Railway CLI
...
## 🏗️ Infrastructure & API Gateway

The application uses an automated AWS Infrastructure-as-Code (IaC) pipeline to manage its API Gateway and Backend routing.

### Key Tools (in `backend/infra/`)
- **`sync_gateway.sh`**: The primary deployment script. Generates the OpenAPI spec, compares it for drift, and deploys updates to AWS via CDK.
- **`generate_openapi.py`**: Dynamically introspects the Flask app to generate an OpenAPI 3.0 specification with API Gateway integration extensions.
- **`check_drift.sh`**: A monitoring utility to verify if local backend routes match the deployed AWS configuration.
- **`cdk/`**: The AWS Cloud Development Kit stack that provisions the API Gateway and its stages.

### Deploying Infrastructure Changes
```bash
cd backend
./infra/sync_gateway.sh
```
...
1. **Frontend (React)**: User interface for chat and search.
2. **API Gateway (AWS)**: Entry point for all requests, handling routing and CORS.
3. **Backend (Flask)**: Core business logic, Google OAuth integration, and orchestration.
4. **Vector Store & Database (Weaviate)**: Semantic search engine for discourses and primary storage for user session data and personal chat history.
5. **AI Service (OpenAI)**: Direct SDK integration for embeddings and chat completions.
...
4. **Metadata Lookup**: Backend fetches additional metadata directly from Weaviate
...
- **flask** - Web framework
- **weaviate-client** - Weaviate integration (v4+)
- **openai** - OpenAI API direct SDK
- **aws-cdk-lib** - AWS Infrastructure as Code
- **openapi-spec-validator** - API Spec validation
...
- OpenAI for direct SDK and language models
