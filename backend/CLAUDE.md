...
**`eval_router.py` does not cover the keyword gate.** It calls `plan_queries` directly, and `is_keyword_query` runs *before* that — so a routing change there is invisible to this harness and to `eval_ragas.py` alike (only 7 of 230 golden questions trip the gate, and none of them end up on the keyword route). Use `evals/test_keyword_route.py` for the gate, and check the affected golden ids by hand: q004, q034, q035, q036, q094, q095, q174.
...
- **`search/`** — the pipeline, one module per stage. `config.py` is the single source of truth for every model, threshold and flag, and carries the measurements behind each choice; read it before changing a constant.
  ...
  - **A bare 1-2 word query never reaches the router.** `is_keyword_query` (`query_planning.py`) gates a lexical BM25 route (`keyword.py`) that runs *before* `plan_queries` — the only placement that removes the planner call rather than just the grader. It exists because the router prompt is actively working against a bare topic: rule (9) tells the model to expand it into synonym facets, so "karma" came back as thematically adjacent discourses rather than the ones that use the word, and the grader was then asked whether a passage "answers the question" about something that poses no question. Both LLM calls were being spent making the result worse. Falls through to semantic when fewer than `KEYWORD_MIN_DISCOURSES` discourses match literally — lexical search has no synonyms, which is the point of it and also why it needs the escape hatch. **Thin does not mean discarded.** `keyword_search` returns those literal matches as `thin_discourses` and `_finish` serves them when every other route came back empty, which is the only reason a bare proper noun works at all: "Wolfe Messing" is in 3 passages across 2 discourses, under the threshold of 3, and semantic has no synonyms for a name — it called the query `out_of_domain` and abstained, so the user got nothing for a term the corpus demonstrably contains. Quoting it worked, because that takes the exact-phrase path. A served reserve is `quality: "partial"` (never `strong` — that would be the "thin result dressed up as a thick one" the threshold exists to prevent) and emits `KEYWORD_SERVED_THIN`, **not** `KEYWORD_FELL_BACK`, whose copy promises a meaning search that in this case is precisely what found nothing. `KEYWORD_ROUTE_ENABLED=0` rolls it back.
  - **`_NON_TOPICAL` is the fourth code-side router rule, and it was measured, not guessed.** Greetings and app-test junk ("Hello", "test", "thanks") are short and declarative, so the token-count and question-word rules pass them — and the corpus genuinely contains those words, so BM25 answers confidently. Bare `test` returned 10 discourses led by "Welcome The Tests" against golden-set labels q034/q035 that expect an abstention, which the router delivers correctly every time. What disqualifies these is the *user's intent*, which lexical matching cannot see, so the list has to be explicit. **Do not "simplify" it away** — the fix is always to fall through to the router, never to teach the keyword route about intent.
  ...
  - `transparency.py` builds the opt-in trace (`include_trace: true`). The frontend no longer *displays* the trace as a panel — the "How I searched" aside was removed — but it still drives behaviour there: `deferred` gates the two-phase render, `route`/`keyword.term` drive keyword highlighting in the reader, `intent`/`unanswerable_reason` steer follow-up redirects, and the summary line and refinement guidance are both derived from it. Keep emitting it.
