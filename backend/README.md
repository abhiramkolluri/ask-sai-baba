...
**Before the router: the keyword route.** A bare 1–2 word query — `karma`, `truth`, `inner peace` — is a topic to browse, not a question to answer, and it short-circuits *before* `plan_queries` is called (`is_keyword_query`, a pure function in `query_planning.py`). It runs pure BM25 (`search_passages_keyword`), keeps only passages that literally contain the term, puts discourses whose title names it first, and returns — **no planner, no grader, no embedding call, ~0.5s** against ~5s for the semantic route. Fewer than `KEYWORD_MIN_DISCOURSES` literal matches ("thin") falls through to the semantic route, which has synonyms; lexical search does not, and that is both the point of it and why it needs an escape hatch. **The thin matches are not thrown away, though** — they are held in reserve and served if every other route also comes up empty (`pipeline.py::_finish`), because a proper noun the corpus mentions once or twice would otherwise return nothing at all: semantic has no synonyms to offer for a name, so it lands on `out_of_domain` and abstains. A reserve result is marked `quality: "partial"` and carries the `KEYWORD_SERVED_THIN` reason, distinct from `KEYWORD_FELL_BACK` — the meaning search is exactly what returned nothing, so telling the user it ran instead would be backwards. Rolls back with `KEYWORD_ROUTE_ENABLED=0`.

> **`trace.route` and `trace.keyword.term` are a contract with the frontend.** When the route is `keyword`, the discourse page highlights *every* occurrence of the term instead of the single best-answer sentence, and offers a find-in-page control. Note that `trace.keyword` is also present with `route: "semantic"` on a fall-back, which the frontend must not treat as a keyword result — renaming either field, or setting `route` without `keyword.term`, breaks that page. See the "Discourse highlighting" section of `ask-sai-vidya-frontend/README.md`.
